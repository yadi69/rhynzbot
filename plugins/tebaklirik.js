...
`.trim()
    conn.tebaklirik[id] = [
        await conn.send2Button(m.chat, caption.trim(), '© rhynz', 'BANTUAN', `${usedPrefix}teli`, 'NYERAH', 'nyerah'),
        json, poin,
        setTimeout(async () => {
            if (conn.tebaklirik[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© rhynz', 'TEBAK LIRIK', `${usedPrefix}tebaklirik`)
