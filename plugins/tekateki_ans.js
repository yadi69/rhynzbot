...
handler.before = async function (m) {
   ...
   if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*tete/i.test(m.quoted.contentText)) return !0
    ...
    if (/^(me)?nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.tekateki[id][1].jawaban))}`.trim(), '© rhynz', 'TEKA TEKI', ',tekateki').then(() => {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        ...
        await this.sendButton(m.chat, `*Benar!* +${this.tekateki[id][2]} XP`, '© rhynz', 'TEKA TEKI', '.tekateki')
