...
`.trim()
    ...
    /**
    * Send Video Faster
    * Taken from https://github.com/ariffb25/stikerinbot
    * @param {String} jid 
     ...
     */
     async sendButton(jid, content, footer, buffer = null, button = [], options = {}) {
      ...
      if (Buffer.isBuffer(buffer)) {
        const m = await this.prepareMessage(jid, buffer, MessageType.image)
        buttonMessage['headerType'] = 4
        buttonMessage['imageMessage'] = m.message.imageMessage
      }
